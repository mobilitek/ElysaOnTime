...
type Entry = { id: string; clientId: string; projectId: string; clientName: string; projectName: string; workDate: string; durationMinutes: number; clientMinutes: number; description: string; hourlyRate: string; amount: string; isBilled: boolean; isDeleted: boolean };
...
export function WorkLogPage(props: Props) {
  ...
  const [editing, setEditing] = useState<Entry | null>(null); const [formOpen, setFormOpen] = useState(false); const [entryClientId, setEntryClientId] = useState(''); const [entryProjectId, setEntryProjectId] = useState(''); const [entryProjects, setEntryProjects] = useState<Project[]>([]); const [workDate, setWorkDate] = useState(iso(today)); const [time, setTime] = useState('08:00'); const [clientTime, setClientTime] = useState('08:00'); const [description, setDescription] = useState(''); const [error, setError] = useState('');
  ...
  const entryClient = clients.find((item) => item.id === entryClientId);
  const entryUsesBank = Boolean(
    entryClient?.hourBankEnabled
    && entryClient.hourBankStartDate
    && workDate >= entryClient.hourBankStartDate,
  );
  ...
  const save = async (event: FormEvent) => {
    ...
    const completedTime = completeWholeHours(time);
    const completedClientTime = entryUsesBank ? completeWholeHours(clientTime) : null;
    const suggestions = [
      completedTime ? `${time.trim()} → ${completedTime}` : null,
      completedClientTime ? `${clientTime.trim()} → ${completedClientTime}` : null,
    ].filter(Boolean);
    if (suggestions.length && !confirm(`${text.completeHours}\n\n${suggestions.join('\n')}`)) return;
    const normalizedTime = completedTime ?? time;
    const normalizedClientTime = completedClientTime ?? clientTime;
    if (completedTime) setTime(completedTime);
    if (completedClientTime) setClientTime(completedClientTime);
