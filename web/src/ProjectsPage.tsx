...
export function ProjectsPage({ language, user, systemInfo, systemInfoError, onLanguageChange, onLogout, onNavigateWorkLog, onNavigateClients, onNavigateProfile, onNavigateAdmin }: Props) {
  ...
  return <main className="app-page">
    ...
    <section className="content-shell">
      <div className="page-heading"><div><h1>{text.title}</h1><p>{text.subtitle}</p></div>{clientId ? <button className="add-button" type="button" onClick={openCreate}><span>+</span>{text.add}</button> : null}</div>
