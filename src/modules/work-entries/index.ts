...
import { BilledEntryAssignmentError, ClientTimeLimitError, createEntry, duplicateEntry, DuplicateTargetOccupiedError, EntryNotFoundError, InvalidDescriptionError, InvalidDurationError, listEntries, ProjectUnavailableError, toggleEntries, updateEntry } from './service';
...
const errors = (error: unknown, status: (code: number, body: unknown) => unknown) => {
  ...
  if (error instanceof BilledEntryAssignmentError) {
    return status(409, { error: 'BILLED_ENTRY_ASSIGNMENT_LOCKED' });
  }
...
export const workEntryRoutes = new Elysia({ prefix: '/api/work-entries' })
  ...
  .patch('/:id', async ({ body, cookie, params, status }) => { const user = await userFor(cookie[SESSION_COOKIE_NAME].value); if (!user) return status(401, { error: 'UNAUTHENTICATED' }); if (!hasFullAccess(user)) return status(403, { error: 'SUBSCRIPTION_REQUIRED' }); try { return { entry: await updateEntry(user.id, params.id, body) }; } catch (error) { return errors(error, status); } }, { params: t.Object({ id: t.String({ format: 'uuid' }) }), body: t.Intersect([entryBody, t.Object({ projectId: t.String({ format: 'uuid' }) })]) })
